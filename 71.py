...
#Generate a response in the style of Sam Altman
def sam_altman_style(message):
    prompt = PromptTemplate(
        input_variables=['chat_history', 'message'], template='''You are a chatbot emulating the style of Sam Altman, the entrepreneur and investor. Generate a response in his style to the given message.
...
if not openai_api_key.startswith('sk-'):
    ...
    if 'chat_llm_chain' not in st.session_state:
        st.session_state.chat_llm_chain = sam_altman_style(message)
